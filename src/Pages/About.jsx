import React, { useState, useEffect } from "react";
import Loading from "../Components/Loading";
import Timeline from "../Components/Timeline";
import Skills from "../Components/Skills";
import "./about.css";

const About = () => {
  const [data, setData] = useState(null);

  useEffect(() => {
    fetch("/api/api.json")
      .then((response) => response.json())
      .then((json) => setData(json.sobre));
  }, []);

  return (
    <section className="content interna">
      <header className="header-interna">
        <h1 className="title-tag">{data ? data.titulo : ""}</h1>
      </header>
      <div className="conteudo">
        <div className="row">
          <article className="sobre col-lg-6">
            <div className="d-flex">
              <div className="icon-subtitle">
                <i className="fas fa-torii-gate"></i>
              </div>
              <h2>Um Samurai</h2>
            </div>
            {data && data.descricao.length ? (
              data.descricao.map((text, index) => <p key={index}>{text}</p>)
            ) : (
              <Loading />
            )}
          </article>

          <article className="curriculo col-lg-6">
            <div className="row">
              <div className="col-md-6">
                {data ? <Timeline infos={data.experiencia} /> : <Loading />}
              </div>
              <div className="col-md-6">
                {data ? <Timeline infos={data.educacao} /> : <Loading />}
              </div>
            </div>
          </article>
        </div>

        <article className="skills mt-4">
          <div className="d-flex align-items-center justify-content-center">
            <div className="icon-subtitle">
              <i className="fas fa-user-cog"></i>
            </div>
            <h2>Skills</h2>
          </div>
          {data ? <Skills skills={data.skills} /> : <Loading />}
        </article>
      </div>
    </section>
  );
};

export default About;
